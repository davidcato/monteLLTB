...
contains
	...
	function maimag_c(xin)
		...
		complex(dl) :: xin
		real(dl) :: maimag_c
		maimag_c = dble(cmplx(0.d0,-0.5d0)*(xin-conjg(xin)))
	...
	function msqrt(xx)
		...
		tanin = maimag(xx)/mreal(xx)

		msqrt=sqrt(xx)
		tanout = maimag(msqrt)/mreal(msqrt)

		! which quadrant?
		atanout=atan(tanout)+ (0.5d0 - mreal(msqrt)/abs(mreal(msqrt)*2.d0))*pi
		if(atanout>pi)atanout=atanout-2.d0*pi
		if(atanout<-pi)atanout=atanout+2.d0*pi

		atanin=atan(tanin)+ (0.5d0 - mreal(xx)/abs(mreal(xx)*2.d0))*pi
        ...
        ! sortcomplexes: sorts an array xin(:): first reals large to small, than complexes based on real part. Optional norm: if im(xin(i))/norm < epsilon, xin is made real.
	subroutine SortComplexes(xin,norm)
		implicit none
		complex(dl), intent(inout) :: xin(:)
                real(dl), intent(in), optional :: norm
                complex(Dl) :: myx1(size(xin)), myx2(size(xin)), myx3(size(xin)), dummy
...
!			if(  abs(maimag(xin(i)))/abs(mreal(xin(i))) < accuracy(dl) *acceptance_pure) then
                   if(present(norm))then
                      call MakeRealIfReal(xin(i),accuracy(dl) *acceptance,norm)!_pure)
                   else
                      call MakeRealIfReal(xin(i),accuracy(dl) *acceptance)!_pure)
                   end if
                   if(  abs(maimag(xin(i))) < accuracy(dl) ) then ! ==0.d0 should work now too.
                      isreal(i) = .true.
!                      xin(i) =  killimag(xin(i)) ! Unnecessary after checkifreal
                      nreals = nreals + 1
                   end if
		end	do
        ...
        subroutine MakeRealIfReal(myval,prec,norm)
          implicit none
          complex(dl), intent(inout) :: myval
          real(dl), intent(in) :: prec
          real(dl), intent(in), optional :: norm
          real(dl) :: thisnorm

          if(present(norm))then
             thisnorm =max(abs(norm),abs(mreal(myval))) ! Take max to be sure.
          else
             thisnorm = abs(mreal(myval))
          end if

          if(abs(maimag(myval))/thisnorm<prec)then
             myval =  killimag(myval)
          end if

        end subroutine MakeRealIfReal
	...
	function Get_Smallest_posrealval(arr_c,index)
		...
		n=size(arr_c)
                ...
                thisindex=-1
		smallestposrealroot = infinity !1.e300_dl
			posrealroots=0
			do i = 1, n
