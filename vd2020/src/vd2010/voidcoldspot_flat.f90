...
!  open(185,file="coldspot_of_z.txt")
  ...
  Lmem=VP%L
  do ii = 1,  50 ! gives only z=1089 if i = 1,1
!     thisz = 1100.d0*(1._dl-dble(ii)/100._dl)
     thisz=1089.d0
     VP%L = dble(ii)/10.d0*Lmem
...
!  VoidE_tau = VoidE_tau +  abar* ( -2._dl*(Sx/abar)**2*Hbar  + 2._dl*(ax/abar)**2 * Hbar)
...
!!$           (-2*adbar**2 + abar*addbar)*Sx**2))/abar**2 
...
!  real(dl) :: vars(3), dvars(3)
  ...
  intvarexactlist=stopval
  nreps = 1.d-4!VoidDistPrec!1.d-4!
...
!  do j = 1, jmax
     ...
     do i = 1, imax
        ...
        newtg = VoidColdNewtGauge(vp%t1100,r=r)
...
!        geodt = VoidColdGeodesic(vp%t1100,r=r,norestore=.true.)
        ...
        write(184,'(5ES24.15,F5.1,5ES24.15)')r/VP%L,r,rFLRW,newtg/3.*2.726d6,(rhox-rhobar)/rhobar, thisz!, geodt
        write(*,'(A)', advance="no")'.'
     end do
     writE(*,*)''
     write(184,*)''
