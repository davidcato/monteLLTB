...
class metric_functions(object):
    ...
    def __init__(self, dataframe_LLTB):
        ...
        # evualte if there is duplicate elements, unlikely to happens.
        try:
            possible_to_interpolate = interpolate.interp1d(dataframe_LLTB['z'],dataframe_LLTB['r'],fill_value='extrapolate')
        except:
            dataframe_LLTB = dataframe_LLTB.drop_duplicates(['z'])
            dataframe_LLTB = dataframe_LLTB.reset_index()
        ...
        self.zr_LLTB = interpolate.interp1d(self.df['r'],self.df['z'],fill_value='extrapolate')
        ...
        self.Rz_LLTB = interpolate.interp1d(self.df['z'],self.df['R'],fill_value='extrapolate')
        self.R0r_LLTB = interpolate.interp1d(self.df['r'],self.df['R0_r'],fill_value='extrapolate')
...
class derivate_functions(metric_functions):
    ...
    def deltar_LLTB(self,r):
        Omout = (1.-self.OmDE-self.OmK)
        dlt = -1 + self.OmMr_LLTB(r)*self.H0r_LLTB(r)**2/(Omout*self.H0r_LLTB(self.rboundary)**2)
        return dlt
