...
class Likelihood(object):
    ...
    ###################################
    # SNe functions stolen from Likelihood_sn
    ###################################

    #class Likelihood_sn(Likelihood):
    def read_matrix_sn(self, path):
        """
    ...
    def read_light_curve_parameters_sn(self, path):
        """
    ...
    def read_light_curve_parameters_sn_low(self, path):
        """
        Read the file jla_lcparams.txt containing the SN data
        ...
        lc_parameters = read_table(
            path, names=names, header=0, index_col=False)
        return lc_parameters
...
class Likelihood_clik(Likelihood):
    ...
    def loglkl(self, cosmo, data, LLTBin):
        ...
        effective_frw = LLTBin
        # selecting just efffective FLRW parameters
        nonsense_cosmo = effective_frw[0]

        if nonsense_cosmo == 0:

            ############################################
            # Passing effective FLRW to CLASS
            ############################################
            nuisance_parameter_names = data.get_mcmc_parameters(['nuisance'])

            params_eff = effective_frw[-1]
            ...
            newcosmo_arguments = {'lensing': 'yes', 'output': 'tCl lCl pCl', 'l_max_scalars': self.l_max, 'h': h_eff, 'Omega_k': OmegaK_eff, 'tau_reio': tau_reio_in,
                                  'omega_cdm': omega_dm_eff, 'omega_b': omega_b_eff, 'T_cmb': T0_eff, 'N_ur': N_ur, 'ln10^{10}A_s': logA_s_in, 'n_s': n_s_in}
            ...
            # cls from effective FLRW
            cl = self.get_cl(newcosmo)
